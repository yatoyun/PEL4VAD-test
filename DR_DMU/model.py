...
class WSAD(Module):
    def __init__(self, input_size, a_nums, n_nums,dropout=0.1):
        ...
        # self.relu = nn.ReLU()
        self.hard_atten = HardAttention(k=0.95, num_samples=100, input_dim=input_size//2)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # x = torch.cat((x_c, (torch.cat([N_aug_new + A_Naug, A_aug_new + N_Aaug], dim=0))), dim=-1)
            ...
            # x = torch.cat([x_c, A_aug + N_aug], dim=-1)
