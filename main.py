...
def train(model, train_nloader, train_aloader, test_loader, gt, logger):
    ...
    initial_auc, initial_ab_auc = test_func(
        test_loader, model, gt, cfg.dataset, cfg.test_bs
    )
    logger.info(
        "Random initialize AUC{}:{:.4f} Anomaly AUC:{:.5f}".format(
            cfg.metrics, initial_auc, initial_ab_auc
        )
    )
...
def main(cfg):
    ...
    if args.mode == "train":
        global logger_wandb
        name = "{}_{}_{}_{}_Mem{}_{}".format(
            args.dataset, args.version, cfg.lr, cfg.train_bs, cfg.a_nums, cfg.n_nums
        )
        logger_wandb = wandb.init(
            project=args.dataset + "(clip+i3d)",
            name=name,
