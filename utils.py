...
def process_feat2_torch(feat, length, is_random=False):
    new_feat = torch.zeros((length, feat.shape[1]), device='cuda')
    r = torch.linspace(0, len(feat), length+1, dtype=torch.int32, device='cuda')
    for i in range(length):
        if r[i] != r[i+1]:
